...
def create_engine_for_url(database_url: str | None = None, *, echo: bool = False) -> Engine:
    """Return a fresh SQLAlchemy Engine per call.

    Engine reuse/caching was tried but reverted: under Streamlit's per-session
    worker threads, a shared SQLite engine crash-looped the server (hard exit,
    no Python traceback). A fresh short-lived engine per session_scope() is the
...
def dispose_all_engines() -> None:
    """No-op retained for API/test compatibility (engine caching was reverted)."""
    return None
