...
def show() -> None:
    ...
    # dropdown.
    # One renderer since 2026-08-06 (owner retired legacy_server and topology_v1),
    # so there is nothing to choose between. The ?sld_dev picker and its warnings
    # existed only to expose the alternatives; a select box with one option is
    # furniture, not a control.
    renderer_mode = DEFAULT_SLD_RENDERER_MODE
    st.caption(f"Renderer mode: {sld_renderer_mode_label(renderer_mode)}.")
