...
## 四之十八、0.1 MWh 的来源追查（owner 追问 2026-08-08）

owner 两次追问：「0.1MWh 的计算差异是不是还没找到原因？」「不可能无中生有」。
据此把「什么机制能真的产生 ~0.1 MWh 偏差」逐条量了一遍。**全部为 0。**

### 18.1 逐条排除（实测，非推断）

| 可能机制 | 实测结果 |
| --- | --- |
| §1 与 §5 读到不同的数 | 同一个 DataFrame 的同一格（探针验证 `ctx.stage3_df is stage13_output["stage3_df"]` 为真） |
| 持久化 → 恢复的往返误差 | `+0.000000000000 MWh`；21 行中无一列发生变化 |
| 已删除的 Excel 重算路径（四之十五） | 同曲线下 `+0.000000 MWh`，无一列变化 |
| 报告目标 ≠ 引擎目标 | 产品里唯一写入者 `report_export_view:497` 直接赋成 `poi_energy_req_mwh`，同一个 float |
| 代码史 | `origin/master` 是 2025 年旧原型（`dc_logic.py` / `stage4_app.py`），**没有** `report_v2.py`；当前分支为压缩导入。两条线都到头 |

### 18.2 决定性推论：0.1 从来救不了合格设计

引擎判据是 `poi_g + 1e-6 >= req`，**收敛的设计按定义永远 ≥ 目标**。
所以 0.1 只可能改变**引擎已判不合格**的设计的结论。那种设计短多少？
把扩容循环停在收敛前一步实测：

```
收敛于第 18 轮，裕度        +0.0698 MWh
停在第 17 轮：未收敛，短缺   3.6006 MWh
```

**一步 = 3.6 MWh 量级**，`(0, 0.1]` 这个带子在实际运行中够不到。

收敛裕度分布（10 组配置实测，container_only）：
...
裕度区间 0.07 ~ 2.95 MWh —— 与 0.1 同量级甚至更小。

### 18.3 结论与一处更正

**结论**：0.1 不对应任何真实计算偏差（每条路径实测均为 0）。
最合乎证据的解释是：写的人看到收敛裕度常常只有零点几 MWh，
凭手感加了一个「差不多就算达标」的垫子。但那是**裕度**的尺度，不是**误差**的尺度——
当容差用等于把及格线下移了一个与真实裕度同量级的距离。
这也解释了它为何与 §5、引擎、canon 三者全都不一致：它不是从任何规则推出来的。

**更正四之十五 15.1 的对外后果描述**：原文写「缺口落在 `(0, 0.1]` MWh 的既有设计，
§1 由 Yes 翻成 No」。理论成立，但 18.2 实测表明该带子实际够不到（步长 3.6 MWh 量级）。
**删除 0.1 对既有设计没有实际影响** —— 消除的是潜在自相矛盾，不是收紧任何合同结论。
升级到 V2.2 后不会有客户既有方案的结论翻转。

---

## 四之十九、两日审核统计（2026-08-06 ~ 08-08，收口于 V2.2）
...
| 文件 | 95 changed |
| 合计 | +3,876 / −9,822 |
| **产品代码** | +749 / −8,442（净 **−7,693**） |
| 测试 | +2,137 / −1,375（净 **+762**） |
| 文档 | +990 / −5 |
| 测试总数 | **743 passed, 0 skipped** |
| **冻结正典** | **七个模块 diff 为空 —— 一字节未动** |

产品代码净减 7,693 行、测试净增 762 行，是这两天的形状：**删掉不服务任何人的东西，
给留下来的东西补上锁。**

### 19.2 删除（均先做可达性实测，非读代码推断）

| 对象 | 规模 | 判据 |
| --- | --- | --- |
| pypowsybl / IIDM 栈 | 11 模块 ~2.2k 行 | 闭合集群，仅互相引用 |
| 被取代的渲染器 + `legacy_server` / `topology_v1` | ~5k 行 | owner 裁定退役 |
| 报告校验层（`_validate_*` / `qc_checks` / `validate_report_context`） | ~200 行 | 产品零调用，且混合站算术错误 |
| `create_ac_report` / `create_combined_report` + 仅其可达者 | 17 函数 413 行 | AST 可达性：无产品调用者，无锚定作用 |
| `_get_stage3_df`（Excel 重算） | — | 探针实测两条路径各 0 次；违反 canon 权威边界 |
| 死 import（`ac_view` ×5、`dc_view` ×2、`site_layout_view` ×1、测试 ×1） | 9 处 | AST + grep 双重核实 |

### 19.3 修正（每项都有可复现的实测）

| 缺陷 | 量化证据 |
| --- | --- |
| §3 公式陈述与自己印的数字对不上 | 读者复算 472.49 vs 报告 458.09 MWh，差 3.1% |
| §1 保证判据私设 0.1 容差 | 全库六处判据中唯一不守 canon 者；同一文档 §1 说 met、§5 说 No |
| 混合站校验算术 | 92 DC 用例误报「expected 96, got 92」「120 MW vs 115 MW」 |
| 导出改写活 Stage 3 | 副本化前会往 session 里加 5 个 NaN 列 |
| DC 页面对同一 mode 跑两遍 sizing | 显示与入库来自两次独立执行 |
| 服务器保留策略未进容器 / 版本号不随升级变 | 见四之四、四之二 |

### 19.4 加固

- 读取重试 + 「读不到」与「没有」分离（`retry_read` / `load_artifact_bytes_with_failures`）
- **取到才生成**的导出闸门（fail-closed，带重试）
- Site Layout 不再缓存失败读取
- 158.8 MB → 1.0 MB 无引用产物清理，用产品自己的 sweep

### 19.5 自纠（本轮我自己引入后又查出的）

| 缺陷 | 性质 |
| --- | --- |
| 失败清单跨图串用 | layout 读失败会让 §7 声称 SLD「存在但读不出」，而 SLD 从未生成 |
| `FileNotFoundError` 被当瞬时失败重试 3 次 | 已消失的文件被报成「存在但读不出」 |

两者都与「要修的那类假陈述」同性质、方向相反 —— 记录于此，因为
**审查者自己引入的缺陷最容易被自己的叙述盖过去**。

### 19.6 方法

每一项修复都做**反向验证**：撤掉修复，对应测试必须失败。
每一项删除都先用 AST 可达性或运行探针证明不可达。
每一项对外结论都从**真 DOCX / 真页面**里读回来核对，而不是断言代码常量。
