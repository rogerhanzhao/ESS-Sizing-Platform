...
## 二、待 owner 确认的一点（未擅自改动）

**DC Block 门面与间距**。owner 指出"DC Block 只有一个宽面是不开门的，
只有那面可以预留小一点的位置"。

- 现状 `ac_block_arrangement_v2.py`：`dc_pair_gap_m=0.30` 用于**背靠背的无门宽面**
  —— 这一条**符合**该规则。
- 但 `pair_to_pair_gap_m=0.9` 是**相邻镜像对之间**的间距，两侧是 DC 箱的
  **短端面（2.438 m）**；知识文档 §2.4 记载该端面是**设备端（液冷机组 + 风机格栅）**。
  0.9 m 是否足够该端面的检修/散热，需 owner 或产品口径确认。

→ **请 owner 明确端面（设备端/液冷段）所需净距**，我再据此调整；
在得到答复前不改这个数。
