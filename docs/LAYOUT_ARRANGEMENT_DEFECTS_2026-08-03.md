...
## 二之二、8 台 DC 的四面环绕排布（owner 2026-08-03 指示）

> "当一个 AC Block 配置 8 台 DC Block 时，可以将 DC Block 不止在 AC Block 左右两侧。"

实现为 `compute_quad_layout()`（`QUAD_LAYOUT_VARIANT`）：中央竖置 40 ft AC Block，
**N / W / E / S 各一个镜像对（各 2 台）**。每台 DC 的门面都朝一条 3.0 m 通道，
仅无门宽面以 0.30 m 背靠背——完全符合上表的四面净距定则。

| 方案 | 包络 | 占地 | 说明 |
|---|---|---|---|
| 线性一排（旧 §8） | 48.42 × 5.18 m | 251 m² | 细长带状，与产品实物不符 |
| 双侧 4+4（受管） | 18.79 × 15.12 m | **284 m²** | 最省地、电缆最短 |
| **四面环绕（本次采用）** | 18.79 × 28.54 m | **536 m²** | DC 分布四面；多两条 3.0 m 通道 |

**代价如实记录**：四面环绕比双侧 4+4 多占约 **252 m²（约 +89%）**，且南北两对的
直流电缆需绕过站体。受管运行（`layout_variant == BILATERAL`）仍保持其**已确认的
4+4**不变，只有**通用 8PCS/8DC 运行**采用四面环绕。若后续要改回双侧，
只需切换 `report_v2` 中的一处引擎选择。
