...
- **L3 / P2 整站几何图**：一字型双侧块（站体在中央）沿轴成行的真实几何整站图。
  ...
  **上面那张"每块占地"表就是它的定量依据**：一旦这三条定了，L3 可以直接把
  A 方案的 349 m²/块 当作目标值来校核。
- **每行几块的寻优**：现模型固定"每行 2 块 + 共用 MV 走廊"。用同一套账算了
  1/2/3/4 块每行（13 块站）：**416 / 416 / 462 / 495 m² 每块**，
  2 块/行确实最优（站越深，顶底两条 6.0 m 消防道摊得越薄）。
  所以现模型不用改；这里记录下来，免得下次又去"优化"它。
- **占地的硬下限**：设备本身 147.9 m²/块，现方案 349 m²/块，设备占比 42.4%。
  纯 DC 场理论下限 32.7 m²/台，现方案 43.6 m²/台。剩下的差距全在站体开销
  （5.438 m × 行深，省不掉）和消防道/围界（规范强制）。**再往下压需要
  owner 先松动规则档，不是排布能解决的。**
