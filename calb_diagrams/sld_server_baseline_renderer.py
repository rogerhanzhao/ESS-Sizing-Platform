...
"""
Server-baseline SLD renderer.

This file is intentionally copied from commit 8568af4 to preserve the drawing
logic that is currently stable on the production server. It is a comparison and
fallback renderer, not the target engineering_v2 implementation.
"""
...
def _draw_dc_switch(
    ...
    *,
    ...
    bottom_y = (y + h) if lead_end_y is None else max(lead_end_y, y + h)
    span = max(1.0, bottom_y - y)
    contact_r = max(1.8, min(2.4, span * 0.035))
    blade_dx = max(14.0, min(20.0, span * 0.24))
    fuse_w = max(10.0, min(14.0, span * 0.18))
    fuse_h = max(14.0, min(20.0, span * 0.24))

    upper_contact_y = y + max(8.0, min(12.0, span * 0.16))
    lower_contact_y = y + max(24.0, min(32.0, span * 0.40))
    fuse_top = y + max(38.0, min(46.0, span * 0.58))
    fuse_bot = fuse_top + fuse_h
    if fuse_bot > bottom_y - 4.0:
        fuse_bot = bottom_y - 4.0
        fuse_top = max(lower_contact_y + 6.0, fuse_bot - fuse_h)
    ...
    _draw_line_anchored(
        ...
        (x, fuse_top),
        class_="thin",
        start_anchor=(x, lower_contact_y),
        end_anchor=(x, fuse_top),
    )
    dwg.add(dwg.rect(insert=(x - fuse_w / 2.0, fuse_top), size=(fuse_w, fuse_bot - fuse_top), class_="outline"))
    dwg.add(dwg.line((x, fuse_top), (x, fuse_bot), class_="thin"))
    _draw_line_anchored(
        dwg,
        (x, fuse_bot),
        (x, bottom_y),
...
def render_sld_pro_svg(
    ...
    if compact_mode:
        ...
        pcs_box_w = min(116.0, max(92.0, slot_w * 0.36))
        pcs_box_h = 58.0
    ...
    dc_block_layouts: list[dict[str, float | int | str]] = []
    ...
    if compact_mode:
        ...
        dc_interface_y = pcs_y + pcs_box_h + 80
        skid_h = max(380.0, dc_interface_y - skid_y + 60)

        battery_y = skid_y + skid_h + 40
        battery_title_h = 20
        dc_box_h = 76.0
        dc_box_w = min(106.0, max(88.0, slot_w * 0.34))
        dc_box_y = battery_y + battery_title_h + 58.0
        dc_label_h = 34.0
        block_gap_x = 26.0
        block_gap_y = 28.0

        block_index = 0
        max_block_bottom = dc_box_y
        for p_i in range(pcs_count):
            count = (
                max(0, _safe_int(spec.dc_blocks_per_feeder[p_i], 0))
                if p_i < len(spec.dc_blocks_per_feeder)
                else 0
            )
            if count <= 0:
                continue
            columns = 1 if count == 1 else 2
            total_w = columns * dc_box_w + (columns - 1) * block_gap_x
            start_x = pcs_centers[p_i] - total_w / 2.0 + dc_box_w / 2.0
            for local_index in range(count):
                row = local_index // columns
                col = local_index % columns
                center_x = start_x + col * (dc_box_w + block_gap_x)
                block_y = dc_box_y + row * (dc_box_h + dc_label_h + block_gap_y)
                label = f"DC Block #{block_index + 1}" if dc_blocks_total > 1 else "DC Block"
                dc_block_layouts.append(
                    {
                ...
                max_block_bottom = max(max_block_bottom, block_y + dc_box_h + dc_label_h)
                block_index += 1

        if not dc_block_layouts and dc_blocks_total > 0:
            center_x = pcs_centers[0]
            dc_block_layouts.append(
                {
                    "block_index": 0,
                    "pcs_index": 0,
                    ...
                    "width": dc_box_w,
                    "height": dc_box_h,
                    "label": f"DC Block x {dc_blocks_total}",
                }
            )
            max_block_bottom = dc_box_y + dc_box_h + dc_label_h

        battery_note_y = max_block_bottom + 18
...
"""
    ...
    for idx in range(pcs_count):
        ...
        if compact_mode:
            ...
            label_x = pcs_left_x + pcs_box_w + 8.0
            dwg.add(
                dwg.text(
                    f"PCS-{idx + 1}",
                    insert=(label_x, pcs_y + 20),
                    class_="label",
                    text_anchor="start",
                )
            )
    ...
    if compact_mode:
        ...
        # Draw one DC interface per PCS. This is a single-line diagram, so do
        # not draw separate DC A/B busbars or imply two external DC circuits.
        for idx in range(pcs_count):
            cx = pcs_centers[idx]
            switch_top_y = pcs_y + pcs_box_h + 8.0
            _draw_line_anchored(dwg, (cx, pcs_y + pcs_box_h), (cx, switch_top_y), class_="thin")
            _draw_dc_switch(dwg, cx, switch_top_y, 64.0, lead_end_y=dc_interface_y)
            dwg.add(dwg.text(f"F{idx + 1}", insert=(cx + 18.0, switch_top_y + 50.0), class_="small"))
        ...
        layouts_by_pcs: dict[int, list[dict[str, float | int | str]]] = {}
        for layout in dc_block_layouts:
            layouts_by_pcs.setdefault(int(layout["pcs_index"]), []).append(layout)

        for p_idx, layouts in layouts_by_pcs.items():
            if p_idx >= len(pcs_centers):
                continue
            pcs_cx = pcs_centers[p_idx]
            layouts = sorted(layouts, key=lambda item: int(item["block_index"]))
            first_block_y = min(float(layout["y"]) for layout in layouts)
            branch_y = first_block_y - 28.0
            if len(layouts) == 1:
                block_x = float(layouts[0]["x"])
                block_y = float(layouts[0]["y"])
                _draw_line_anchored(dwg, (pcs_cx, dc_interface_y), (pcs_cx, branch_y), class_="thin")
                if abs(block_x - pcs_cx) > 1e-6:
                    dwg.add(dwg.line((pcs_cx, branch_y), (block_x, branch_y), class_="thin"))
                _draw_line_anchored(dwg, (block_x, branch_y), (block_x, block_y), class_="thin")
            else:
                block_centers = [float(layout["x"]) for layout in layouts]
                _draw_line_anchored(dwg, (pcs_cx, dc_interface_y), (pcs_cx, branch_y), class_="thin")
                dwg.add(dwg.line((min(block_centers), branch_y), (max(block_centers), branch_y), class_="thin"))
                for layout in layouts:
                    block_x = float(layout["x"])
                    block_y = float(layout["y"])
                    _draw_line_anchored(dwg, (block_x, branch_y), (block_x, block_y), class_="thin")

        for layout in dc_block_layouts:
            center_x = float(layout["x"])
            block_y = float(layout["y"])
            block_w = float(layout["width"])
            block_h = float(layout["height"])
            label = str(layout["label"])

            dwg.add(dwg.rect(insert=(center_x - block_w / 2.0, block_y), size=(block_w, block_h), class_="outline"))
            inner_pad = max(10.0, block_h * 0.14)
            usable_h = max(1.0, block_h - inner_pad * 2.0)
            _draw_battery_column(dwg, center_x, block_y + inner_pad, usable_h, 6)
            dwg.add(dwg.text(label, insert=(center_x, block_y + block_h + 14.0), class_="small", text_anchor="middle"))
            dwg.add(
                dwg.text(
                    format_mwh(spec.dc_block_energy_mwh),
                    insert=(center_x, block_y + block_h + 28.0),
                    class_="small",
                    text_anchor="middle",
                )
            )
...
def _draw_battery_column(dwg, x: float, y: float, h: float, rows: int) -> None:
    if rows <= 0 or h <= 0:
        return
    cell_count = max(3, min(rows, 5))
    long_w = 30.0
    short_w = 18.0
    plate_gap = 5.0
    stack_bottom = y + max(1.0, h - plate_gap)
    pitch = (stack_bottom - y) / max(1, cell_count - 1)

    dwg.add(dwg.line((x, y - 4.0), (x, stack_bottom + plate_gap + 4.0), class_="thin"))
    dwg.add(dwg.text("+", insert=(x + long_w * 0.85, y + 2.0), class_="small"))
    dwg.add(dwg.text("-", insert=(x + long_w * 0.85, stack_bottom + plate_gap), class_="small"))

    for row in range(cell_count):
        y_long = y + row * pitch
        y_short = y_long + plate_gap
        dwg.add(dwg.line((x - long_w / 2.0, y_long), (x + long_w / 2.0, y_long), class_="thin"))
        dwg.add(dwg.line((x - short_w / 2.0, y_short), (x + short_w / 2.0, y_short), class_="thin"))
