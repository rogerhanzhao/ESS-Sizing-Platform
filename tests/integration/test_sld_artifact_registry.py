...
def _build_run_bundle(sample_excel_path, db_url: str) -> str:
    bundle = load_dc_excel_bundle_from_path(sample_excel_path)
    defaults = dict(bundle.defaults)
    stage1_inputs = {
        "project_name": "SLD Artifact Registry",
...
def test_sld_artifact_registry_records_traceability(sample_excel_path, tmp_path):
    db_url = f"sqlite:///{(tmp_path / 'sld_artifact_registry.sqlite').as_posix()}"
    run_id = _build_run_bundle(sample_excel_path, db_url)
    ...
    artifact_bundle = render_sld_from_run_bundle(
        ...
        options=SldRenderOptions(group_index=1, override_mode=False),
        actor="registry-tester",
        db_url=db_url,
    )
