...
from calb_sizing_tool.services.external_layout_service import review_external_layout, submit_external_layout_artifact
...
def test_registry_preserves_deterministic_and_ai_artifacts(sample_excel_path, tmp_path):
    db_url = f"sqlite:///{(tmp_path / 'registry_preserve.sqlite').as_posix()}"
    ...
    stage1_inputs = {
        "project_name": "Registry Preserve Test",
    ...
    with session_scope(db_url) as session:
        rows = session.query(ArtifactRegistry).filter_by(sizing_run_id=run_id).all()
        kinds = {row.artifact_kind for row in rows}
        assert "layout_svg" in kinds
        assert "layout_ai_approved" in kinds
