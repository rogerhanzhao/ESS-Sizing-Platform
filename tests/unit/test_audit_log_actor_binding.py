...
from calb_sizing_tool.infra.db.models import AuditLog
...
def test_audit_log_actor_binding(sample_excel_path, tmp_path):
    db_url = f"sqlite:///{(tmp_path / 'audit_actor.sqlite').as_posix()}"
    ...
    stage1_inputs = {
        "project_name": "Audit Actor Test",
        "poi_power_req_mw": 20.0,
        "poi_energy_req_mwh": 80.0,
    ...
    with session_scope(db_url) as session:
        logs = session.query(AuditLog).filter_by(entity_type="sizing_run").all()
        assert any(log.actor == "tester" for log in logs)
